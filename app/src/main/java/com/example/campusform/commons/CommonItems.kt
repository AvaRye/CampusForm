package com.example.campusform.commons
...
class ItemAdapter(val itemManager: ItemManager) : RecyclerView.Adapter<RecyclerView.ViewHolder>(),
    ...
    override fun getItemViewType(position: Int) =
        ItemManager.getViewType(
            itemManager[position].controller
        )
}

fun RecyclerView.withItems(items: List<Item>): ItemManager {
    val itemManager =
        ItemManager(items.toMutableList())
...
class ItemManager(private val delegated: MutableList<Item> = mutableListOf()) :
    ...
    companion object ItemControllerManager {
        ...
        private fun ensureControllers(items: Collection<Item>): Unit =
            items.distinctBy(Item::controller).forEach(
                ItemControllerManager::ensureController)
...
//                Log.e(tag, "content the same: old-> $oldItemPosition new-> $newItemPosition diff: ${oldItem.areContentsTheSame(newItem)}")
        ...
        result.dispatchUpdatesTo(object : ListUpdateCallback {
            override fun onChanged(position: Int, count: Int, payload: Any?) {
                log(
                    tag,
                    "change: pos:$position count:$count payload:$payload"
                )
            }
            ...
            override fun onMoved(fromPosition: Int, toPosition: Int) {
                log(
                    tag,
                    "move: from pos: $fromPosition  to pos: $toPosition"
                )
            }
            ...
            override fun onInserted(position: Int, count: Int) {
                log(
                    tag,
                    "insert: pos: $position count: $count"
                )
            }
            ...
            override fun onRemoved(position: Int, count: Int) {
                log(
                    tag,
                    "remove: pos:$position count:$count"
                )
            }

        })
        observer?.let { result.dispatchUpdatesTo(it) }
    }
